...
if COMBAT_AVAILABLE:
    ...
    covars_df = metadata[['source', 'label']].copy()
...
def plot_pca(X_data, title_prefix):
    ...
    df_plot['source'] = metadata['source'].values
    ...
    # 按 source（批次来源）着色
    plt.subplot(1, 2, 2)
    sns.scatterplot(data=df_plot, x='PC1', y='PC2', hue='source', alpha=0.7, palette='tab10')
    plt.title(f'{title_prefix} - Source_colored')
    plt.xlabel(f'PC1 ({pca.explained_variance_ratio_[0]:.2%})')
    plt.ylabel(f'PC2 ({pca.explained_variance_ratio_[1]:.2%})')
    
    plt.tight_layout()
    plt.savefig(f'../figure/long_{title_prefix}_pca.png', dpi=150)
    plt.show()
...
samples = metadata['sample_id'].tolist() # 样本名

# 构建 DataFrame（注意：基因作为行，样本作为列，跟你的原始矩阵一致）
df = pd.DataFrame(X_final, index=samples, columns=genes)

# 保存为 TSV（sep='\t' 指定制表符）
df.to_csv('../processed_data/long.counts.txt', sep='\t', header=True, index=True)


from sklearn.manifold import TSNE
import umap
def tsne(X_data, title_prefix):
    tsne = TSNE(n_components=2, random_state=42, perplexity=100)  # 小样本设perplexity较小
    ...
    # 按 source（批次来源）着色
    plt.subplot(1, 2, 2)
    sns.scatterplot(data=df_plot, x='tsne1', y='tsne2', hue='source', alpha=0.7, palette='tab10')
    plt.title(f'{title_prefix} - Source_colored')
    plt.xlabel(f't_sne1')
    plt.ylabel(f't_sne2')
    
    plt.tight_layout()
    plt.savefig(f'../figure/long_{title_prefix}_tsne.png', dpi=150)
    plt.show()

def u_map(X_data, title_prefix):
    umap_model = umap.UMAP(n_components=2, random_state=42, n_neighbors=100)  # 小样本调低n_neighbors
    ...
    # 按 source（批次来源）着色
    plt.subplot(1, 2, 2)
    sns.scatterplot(data=df_plot, x='umap1', y='umap2', hue='source', alpha=0.7, palette='tab10')
    plt.title(f'{title_prefix} - Source_colored')
    plt.xlabel(f'UMAP1')
    plt.ylabel(f'UMAP2')
    
    plt.tight_layout()
    plt.savefig(f'../figure/long_{title_prefix}_umap.png', dpi=150)
    plt.show()
